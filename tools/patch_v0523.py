...
s=s.replace('Sync Status · v0.5.2.2','Auto Resume Sync · v0.5.2.3',1)
...
if old_const not in s: raise SystemExit("Drive constants marker not found")
...
if old_let not in s: raise SystemExit("Drive state marker not found")
s=s.replace(old_let,new_let,1)

old_init='function setDriveStatus(t,c=false){$("fileStatus").textContent=t;$("storagePill").textContent=c?"☁ Google Drive Sync":"Sejf lokalny";$("googleBtn").textContent=c?"☁ Google Drive: połączony":"☁ Połącz Google Drive";updateSyncPanel(c?"Połączony":"Niepołączony",c?"sync-state-good":"")}async function initGoogleTokenClient(){while(!window.google?.accounts?.oauth2)await new Promise(r=>setTimeout(r,100));if(!googleTokenClient)googleTokenClient=google.accounts.oauth2.initTokenClient({client_id:GOOGLE_CLIENT_ID,scope:GOOGLE_SCOPE,callback:async r=>{if(r.error)return toast(r.error);googleAccessToken=r.access_token;driveConnected=true;setDriveStatus("Połączono",true);if(pendingCloudRestore){pendingCloudRestore=false;await restoreFromCloud()}else await smartSyncAfterConnect()}});return googleTokenClient}'
new_init='''function setDriveStatus(t,c=false){$("fileStatus").textContent=t;$("storagePill").textContent=c?"☁ Google Drive Sync":"Sejf lokalny";$("googleBtn").textContent=c?"☁ Google Drive: połączony":(localStorage.getItem(LS_DRIVE_AUTO_RESUME)==="1"?"☁ Wznów Google Drive":"☁ Połącz Google Drive");updateSyncPanel(c?"Połączony":"Niepołączony",c?"sync-state-good":"")}
...
function showRememberedDriveState(){if(driveAutoResumeEnabled()&&!driveConnected){$("storagePill").textContent="☁ Drive zapamiętany";$("googleBtn").textContent="☁ Wznów Google Drive";if($("fileStatus")&&$("fileStatus").textContent==="Google Drive niepołączony.")$("fileStatus").textContent="Google Drive skonfigurowany · zostanie wznowiony przy odblokowaniu sejfu.";updateSyncPanel("Zapamiętany · oczekuje na wznowienie","sync-state-warn")}}
...
if old_init not in s: raise SystemExit("Google init block not found")
...
if old_unlock not in s: raise SystemExit("Unlock handler not found")
